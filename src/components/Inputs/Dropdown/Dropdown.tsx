...
export const Dropdown: FC<IDropdownProps> = ({ label, value, options, ...other }) => {
    return (
        <>
            ...
            <select
                className={`w-full rounded-[10px] bg-white dark:bg-graphite ${value ? "text-white" : "text-secondary"} outline-none border-2 border-context-light dark:border-transparent dark:focus:border-primary px-5 py-[16.5px] no-scroll`}
                value={value}
                {...other}
            >
